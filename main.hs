...
main = do 
  [ntosubs, ms, ns, cs, d0s, d1s] <- getArgs
  let (ntosub, m, n, c, d0, d1) = (read ntosubs, read ms, read ns, read cs, read d0s, read d1s)

  let g = graph m
  
  r <- liftM (genrandom g c) getStdGen

  let dists = take (n+1) $ drsoDists ntosub (delta g d1) g r (delta g d0)
